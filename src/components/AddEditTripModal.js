...
export default function AddEditTripModal({ open, onClose, onSave, initData }) {
  ...
  // 대표장소를 type="숙소"인 slot의 place로 자동
  useEffect(() => {
  ...
  return (
    ...
    >
      <div
        style={{
          minWidth: 320, maxWidth: 400, background: "#fff",
          borderRadius: 18, padding: 32, boxShadow: "0 6px 32px #8791bd33",
          display: "flex", flexDirection: "column", gap: 16, position: "relative"
        }}
        onClick={e => e.stopPropagation()}
      >
        <div style={{ fontWeight: 800, fontSize: 22, color: "#3240a8", marginBottom: 8 }}>
        ...
        {/* 대표장소 → 숙소정보로 변경 */}
        <input
          placeholder="숙소정보(해당일 숙소 자동등록)"
          value={form.place || ""}
          style={{ ...inputStyle, background: "#f7fbff", fontWeight: 700, color: "#25467a" }}
          readOnly
        />
        {/* 시간대별 일정 입력 */}
        <div style={{ margin: "10px 0", padding: "12px", background: "#f8fafc", borderRadius: 12 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            <input
              style={inputStyle}
              placeholder="메모/일정 내용"
              value={slot.memo}
              onChange={e => setSlot(s => ({ ...s, memo: e.target.value }))}
            />
            <input
              style={inputStyle}
              placeholder="지도URL (구글지도 퍼가기 주소 붙이면 자동 위경도 추출)"
              value={slot.mapUrl}
            ...
            <input
              ...
              placeholder="장소명 (옵션)"
              value={slot.place}
              onChange={e => setSlot(s => ({ ...s, place: e.target.value }))}
            />
            {/* 유형 셀렉트박스 */}
            <select
              style={inputStyle}
            ...
            >{editingSlotId !== null ? "일정 수정" : "+ 일정 추가"}</button>
          </div>
          {/* 시간대별 일정 카드(수정/삭제) */}
          <div style={{ marginTop: 12 }}>
            {[...form.slots].sort((a, b) => {
              ...
              <div key={s.id} style={{
                background: "#fff", border: "1.2px solid #b6c2df",
                ...
                <div>
                  ...
                  {s.mapUrl &&
                    <a href={s.mapUrl} target="_blank" rel="noopener noreferrer" style={{ color: "#3a77e5", fontSize: 12, textDecoration: "underline" }}>지도 링크</a>}
                </div>
                <div>
                  <button
                    style={{ ...buttonStyle, background: "#e8eafc", color: "#4678ff", padding: "5px 10px", marginRight: 4 }}
                    onClick={() => handleEditSlot(s.id)}
                    type="button"
                  >수정</button>
                  <button
                    style={{ ...buttonStyle, background: "#ffe6e6", color: "#b34040", padding: "5px 10px" }}
